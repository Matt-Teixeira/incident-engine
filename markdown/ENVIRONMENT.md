...
| Var | Purpose |
| --- | --- |
| `APP_NAME` | Identity for self-logging into `util.app_run_logs`. **Must be `incident-engine`.** |
...
| `PG_SSLMODE` | `disable` / `require` / `verify-ca` / `verify-full`. Suite default `require`. |
...
- **Least privilege:** `PGUSER` is `incident_engine_rw` — it owns the `incidents` schema,
  has `SELECT` on `util.app_run_logs` + `stats.acquisition_history`, and `INSERT` on
  `util.app_run_logs` (self-log only). Never point the app at a superuser. Create/alter the
  role once with `db/setup-owner-role.sql` (as a superuser). See the Least-Privilege Rule.
