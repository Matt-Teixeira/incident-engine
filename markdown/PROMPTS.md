...
phase** from the Phase 4 review — classify the confirmed hard-failure messages
(`JOB HALTED`, `NO TUNNEL FOUND`, …) out of `unknown`; must also remove/update the
interim-M2 reason string in `rules.js` R1 and bump `RULES_VERSION`. Cross-app vocabulary
(`connection_regex.js` is `data_acquisition`-owned) — coordinate or upstream.
